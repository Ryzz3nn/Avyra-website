...
router.post('/ingest', requireApiKey, async (req, res) => {
    const { discordId, citizenid, charinfo, job, money, playerName } = req.body;
    ...
    const upsertQuery = `
        INSERT INTO characters (discord_id, citizenid, charinfo, job, money, player_name, last_updated)
    ...
    try {
        await webDB.query(upsertQuery, [discordId, citizenid, JSON.stringify(charinfo), JSON.stringify(job), JSON.stringify(money), playerName]);
